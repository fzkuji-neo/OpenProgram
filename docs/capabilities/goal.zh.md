...
已失去执行者的中断执行仍有外部结果未知时，同一会话的新操作需要针对本次操作的单次审批，bypass 模式和既有 allow 规则也不能免除。真正的内置文件核查工具（`read`、`grep`、`glob`、`list`）可按原有权限读取状态；自定义工具仅使用相同名称不会获得只读信任。审批列出此前结果未知的操作及新操作；未知操作集合改变后必须重新审批。批准不把旧操作标为成功或失败，非交互任务不能批准新副作用。此权限检查尚未启用受限自动恢复，也尚未解除未知外部效果对应的 Resume 限制。
