...
执行失败或取消后停止自动续轮。worker 重启本身不再暂停聊天 Goal：兼容的检查点按自动恢复策略继续，停机时已成功结束的轮次保留持久续轮通知。用户暂停、取消、预算耗尽和显式恢复限制仍然阻止自动工作。旧的可恢复暂停状态及缺少已保存恢复来源的 Goal 仍需显式 Resume。未知外部效果和通用外部事件唤醒仍是恢复限制。等待或尚未结束的执行不代表目标完成。
