...
Server 首次使用时启动，之后在进程生命周期内缓存复用，OpenProgram 退出时关闭。Server 进程走 OpenProgram 启动子进程的同一条路径，所以配置的[沙箱](../reference/design/runtime/sandbox-architecture.html)对它同样生效：默认的 `workspace-write` 模式下，language server 只读工作区、不往外写，这正好够用。
