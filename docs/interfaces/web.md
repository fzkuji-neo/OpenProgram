...
The floating webpage preview fills the space below its compact header, with rounded corners and without an outer frame or side and bottom padding. Drag the header to move it; use its edges or enlarged corner targets to resize it. A curved hint follows the window corner directly when hovering over it.
...
Press a top tab to highlight it; release without dragging to switch pages. Dragging keeps the current page visible. Drop outside the strip to use the existing new-window transfer. The New window cue stays in the tab strip above native webpages. To add an open webpage to a conversation, keep that conversation visible and drag the webpage tab onto **Resources** (or its open panel). Release to attach the same page. Its top tab and existing associations stay available; a failed attachment leaves the page unchanged.
