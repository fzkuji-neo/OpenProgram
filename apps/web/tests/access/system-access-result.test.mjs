...
test('only structured blocked results expose registry system capabilities', () => {
 ...
 assert.deepEqual(systemAccessRequired(JSON.stringify(value)),['screen_recording','arbitrary']);
...
test('registry capability ids are preserved in system-access results', () => {
 const value = {status:'infeasible', reason_code:'system_access_required', system_access:[
   {id:'calendar', status:'not_granted'},
   {id:'microphone', status:'unknown'},
   {id:'screen_recording', status:'not_granted'},
 ]};
 assert.deepEqual(systemAccessRequired(value), ['calendar','microphone','screen_recording']);
});
